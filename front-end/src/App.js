...
function App() {
  ...
  function titleValidation() {;
    const invalidLength = title.length > 12;
    if (invalidLength) {
      setTitleError(true);
      setTitleErrorMessage('The length of the title cannot exceed 12 characters!');
      setValidated(false);
    } else {
      setTitleError(false);
    }
  ...
  function setTitleFromTyping(event) {
    setTitle(event.target.value);
    titleValidation();
  }

  function setTitleFromPasting(event) {
    setTitle(event.clipboardData.getData('Text'));
    titleValidation();
  }

  function detailValidation() {
    const invalidLength = detail.length > 300;
    if (invalidLength) {
      setDetailError(true);
      setDetailErrorMessage('The length of the detail cannot exceed 300 characters!');
      setValidated(false);
    } else {
  ...
  function setDetailFromTyping(event) {
    setDetail(event.target.value);
    detailValidation();
  }

  function setDetailFromPasting(event) {
    setDetail(event.clipboardData.getData('Text'));
    detailValidation();
  }
  ...
  function handleAddTodo(event) {
    ...
    if (detail.length === 0) {
      setDetailError(true);
      setDetailErrorMessage('Detail cannot be empty!');
    }

    // existing Title check
    let titleExists = false
    for (let i = 0; i < todoList.length; i++)
      if (todoList[i].title === title) titleExists = true;
    ...
    const error = titleError || titleExists || detailError;
  ...
  function doneTodo(title) {
    axios.delete(`http://localhost:8000/api/todo${title}`, title)
    .then(setTodoList(todoList.filter(todo => todo.title !== title)))
    .catch(err => console.log(err));
  }
  ...
  return (
    <div className="App">
      <header>
        <div className="logo">My To<span className='main-span'>Do</span> <span className='secondary-span'>App</span></div>
      </header>

      <div className="nav-bar-bg"></div>

      <Form noValidate validated={validated} onSubmit={handleAddTodo}>
        <Form.Label className="mb-2 fs-2">TODO Submission</Form.Label>
        <Form.Group className="mb-3" controlId="formBasicTodo">
          <Form.Control required type="text" placeholder="Enter TODO title"
          onChange={e => setTitleFromTyping(e)} onPaste={e => setTitleFromPasting(e)} isInvalid={titleError}/>
        ...
        <Form.Group className="mb-3" controlId="formBasicDetail">
          <Form.Control required as="textarea" rows={3} placeholder="Detail"
          onChange={e => setDetailFromTyping(e)} onPaste={e => setDetailFromPasting} isInvalid={detailError}/>
          <Form.Control.Feedback type="invalid">
            {detailErrorMessage}
          </Form.Control.Feedback>
        </Form.Group>

        <Form.Label className="mt-3">Priority Level</Form.Label>
        <Form.Select defaultValue="Moderate" className="mb-3" controlId="formBasicDropdown"
      ...
      <TodoList todos={todoList} doneTodo={doneTodo}/>
